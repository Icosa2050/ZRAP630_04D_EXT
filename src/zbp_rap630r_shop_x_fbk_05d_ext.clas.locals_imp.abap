...
CLASS lhc_shop DEFINITION INHERITING FROM cl_abap_behavior_handler.
  ...
  PRIVATE SECTION.
    METHODS zz_validateDeliverydate               FOR VALIDATE ON SAVE
      IMPORTING keys FOR Shop~zz_validateDeliverdate.
...
CLASS lhc_shop IMPLEMENTATION.
  ...
  METHOD zz_validateDeliverydate.
    READ ENTITIES OF ZRAP630i_ShopTP_05D IN LOCAL MODE
            ENTITY Shop
  ...
  METHOD ZZ_setOverallStatus.
    DATA update_bo      TYPE TABLE FOR UPDATE     ZRAP630i_ShopTP_05D\\Shop.
    DATA update_bo_line TYPE STRUCTURE FOR UPDATE ZRAP630i_ShopTP_05D\\Shop .
    ...
    DATA(product_value_help) = NEW zrap630_cl_vh_product_05D(  ).
    ...
    MODIFY ENTITIES OF zrap630i_shoptp_05D IN LOCAL MODE
      ENTITY Shop
        UPDATE FIELDS (
        OverallStatus
        CurrencyCode
